...
let tagsCount, tagsInputBox, tagsList;
let submitBtn;
...
const tagsDeconstructor = (e) => {
    if (e.target.tagName == "BUTTON") { // c(e.currentTarget); The #tag-list element
        tagsCount -= 1;
...
const getInputValue = easyMDE => {
    ...
    var authors = [...document.getElementsByName('authors')].filter(author => {
        return author.checked;
    }).map(author => {
        return author.value
    });
    var category = document.getElementsByName('category')[0].value;
    var tags = [...document.getElementsByName('tags')].filter(tag => {
...
const validateInput = (title, subtitle, date, authors, category, tags, content) => {
    ...
    if (!/^\d\d\d\d-\d\d-\d\d$/.test(date)) {
        ...
        // TODO The regex can't detect 02/30, 04/31 ... . Nevertheless, Beckend will fix this error
        canSubmit = false;
        document.getElementById('err_msg_date').innerText = errInputMsg.dateIllegal;
    } else if (Number(date.split('-')[0]) < 1960) {
        // TODO If the input date is in the current year but in a future month and/or day. Nevertheless, Beckend will fix this error
        canSubmit = false;
...
const uploadMultipleImages = async (url, data) => {
    return response = await fetch(url, { // Do NOT set Content-Type header (browser will set this header automatically)
        method: 'POST',
        body: data
    });
};

const submitArticle = async (imageEndpoint, formData, formNameIDMapping, articleEndpoint, title, subtitle, date, authors, category, tags, content) => {
    var success =  await uploadMultipleImages(imageEndpoint, formData)
...
const submitNewPost = () => {
    ...
    if (!res) {
        ...
        for (const f of fileField) {
            ...
            formData.append('uploadImage', f.files[0]);
        ...
        submitArticle('/admin/create/images', formData, formNameIDMapping, '/admin/create/article', title, subtitle, date, authors, category, tags, content);
    }
};
...
onDOMContentLoaded = (function(){
    ...
    fileGroups.addEventListener('click', (e) => {
        ...
        // c( e.target, e.target.closest('input[type=file]'));
